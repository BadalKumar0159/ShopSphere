...
const Collection = () => {
  ...
  const toggleCategory = (e) => {
    if (category.includes(e.target.value))  // if present, remove it
    {
  ...
  const applyFilter = () => {
    ...
    let productsCopy = products.slice(); //will store only products whose category & subCategory are present

    if(showSearch && search)
      productsCopy = productsCopy.filter(item => item.name.toLowerCase().includes(search.toLowerCase()));
    if (category.length > 0)                  //in category arr & subCategory arr
